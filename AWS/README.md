...
4. Password          : Click the `Store in Keychain` and enter the password 
                       that you determined as "Pass12345" while creating the DB Instance.
...
- Show `myDatabase` database that is created together with RDS DB instance creation.

- Create a new database from "Schema" tab

- To modify the database, first, we need to create a new table. So, click the `myDatabase` schema (or your schema's name). Right-click the `Table` option, then select the `Create Table`, and enter `Personal_Info` as table name.
...
```sql
CREATE TABLE `myDatabase`.`Personal_Info_2` (
...
```sql
INSERT INTO myDatabase.Personal_Info
(ID_number, Name, Surname, Gender)
VALUES
('1234','Nedim','myDatabase','Male'), ('56789','Ekrem','myDatabase','Male'), ('007','Refia','myDatabase','Female');
```

- Write a query to show all data in the `Personal_Info` table

```sql
SELECT * FROM myDatabase.Personal_Info;
```

- Write a query to show the personal whose salary are higher than 40K in the `Personal_Info` table

```sql
 SELECT * FROM myDatabase.Personal_Info WHERE salary > 40000
```
